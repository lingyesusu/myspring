...
public class KickoutSessionControlFilter extends AccessControlFilter {
	...
	protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) throws Exception {
		 ...
		 * 2.如果是普通请求，直接跳转到登录页
		 */
		Boolean marker = (Boolean)session.getAttribute(JedisShiroContant.REDIS_SHIRO_KICKOUT_STATUS);
		...
		LinkedHashMap<Integer, Serializable> infoMap = cache.get(JedisShiroContant.REDIS_SHIRO_ONLINE_USER, LinkedHashMap.class);
		//如果不存在，创建一个新的
		...
		if(infoMap.containsKey(userId) && infoMap.containsValue(sessionId)){
			...
			cache.setex(JedisShiroContant.REDIS_SHIRO_ONLINE_USER, infoMap, 3600);
			return Boolean.TRUE;
		}
		...
		if(infoMap.containsKey(userId) && !infoMap.containsValue(sessionId)){
			...
			if(null != oldSession){
				//标记session已经踢出
				oldSession.setAttribute(JedisShiroContant.REDIS_SHIRO_KICKOUT_STATUS, Boolean.TRUE);
